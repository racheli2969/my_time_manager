...
export const TaskCard: React.FC<TaskCardProps> = ({ task, onEdit, onDelete, onStatusChange }) => {
  ...
  const assignedUser = task.assignedTo ? (typeof window !== 'undefined' ? JSON.parse(window.localStorage.getItem('users') || '[]').find((u: any) => u.id === task.assignedTo) : null) : null;
  // fallback: if context available, use context
  // If you want to use context, you can import useUser and get users from there
  ...
  return (
    <div className={`bg-white border rounded-xl shadow-lg p-6 flex flex-col justify-between min-h-[320px] max-w-md mx-auto transition-shadow ${
