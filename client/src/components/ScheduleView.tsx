...
export const ScheduleView: React.FC = () => {
            ...
            {weekDays.map((day, index) => {
              ...
              return (
                <div key={index} className="min-h-96">
                  <div className="mb-2">
                    <h4 className={`text-sm font-medium ${isToday ? 'text-blue-600' : 'text-gray-900'}`}>
                    ...
                    {!isWorkDay && <p className="text-xs text-gray-500">Non-working</p>}
                  </div>
                  <div className="space-y-2">
                    {dayEntries.length > 0 ? (
                      dayEntries.map(entry => (
                        <div
                          key={entry.id}
                          className={`p-2 rounded-lg cursor-move transition-all hover:shadow-sm ${getPriorityColor(entry.priority)} ${entry.isLocked ? 'opacity-75' : ''}`}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Intelligent Schedule</h2>
          <p className="text-gray-600 mt-1">AI-optimized task scheduling within your work hours</p>
